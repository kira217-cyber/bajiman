...
const LoginModalSetting = () => {
  ...
  const loadSetting = async () => {
    try {
      setPageLoading(true);
      const res = await api.get("/api/login-modal-settings");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await api.put("/api/login-modal-settings", fd, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      toast.success("Login modal setting saved successfully");
      await loadSetting();
    } catch (error) {
  ...
  const removeLogo = async () => {
    ...
    try {
      setLoading(true);
      await api.delete("/api/login-modal-settings/logo");
  ...
  const handleReset = async () => {
    ...
    try {
      setLoading(true);
      await api.patch("/api/login-modal-settings/reset");
      toast.success("Login modal setting reset successfully");
      await loadSetting();
    } catch (error) {
        ...
        <div className="relative flex flex-col justify-between gap-5 lg:flex-row lg:items-center">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm text-slate-300">
              Control login modal logo and all colors from admin panel.
            </p>
          </div>
      ...
      >
          ...
          <Section title="Form Colors">
            <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
              ...
              <ColorInput
                ...
                onChange={(v) => setValue("inputFocusBorder", v)}
              />
              <ColorInput
        ...
        <section className="sticky top-6 h-fit rounded-3xl border border-[#1A79D3]/20 bg-black/35 p-5 shadow-2xl md:p-6">
          ...
          >
            ...
            {loading ? "Saving..." : "Save Login Setting"}
          </button>
        </section>
      </form>
    </div>
    ...
    >
      ...
      <div className="px-5 py-6">
        ...
        <div className="mt-2 text-right">
          <span className="text-sm" style={{ color: form.linkText }}>
            Forgot password?
          </span>
        </div>
        ...
        >
          ...
          <span style={{ color: form.linkText }}>Sign Up</span>
        </div>
      </div>
    </div>
  </div>
  ...
  <div>
    ...
    <input
      ...
      style={{
        backgroundColor: form.inputBg,
        color: form.inputText,
        borderColor: form.inputFocusBorder,
      }}
    />
  </div>
);
